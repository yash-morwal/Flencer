    ...
    <nav className="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl animate-slideDown">
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 lg:px-16 py-3">
        <div className="flex justify-between items-center">
          {/* Logo */}
          <div className="heading-font text-2xl font-bold text-[var(--main-text-color)] cursor-pointer hover:text-blue-500 transition-colors duration-300">
            Flencer.
          </div>
          ...
          <div className="hidden md:flex gap-12 items-center bg-[var(--navbar-menu-bg)] backdrop-blur-lg border border-[var(--border-color)] border-opacity-80 rounded-full px-6 py-2">
            <a href="#home" className="paragraph-font text-[var(--main-text-color)] hover:text-blue-500 transition-colors duration-300 relative group">
              Home
              <span className="absolute bottom-[-5px] left-0 w-0 h-[2px] bg-blue-500 group-hover:w-full transition-all duration-300"></span>
            </a>
            <a href="#projects" className="paragraph-font text-[var(--main-text-color)] hover:text-blue-500 transition-colors duration-300 relative group">
              Projects
              <span className="absolute bottom-[-5px] left-0 w-0 h-[2px] bg-blue-500 group-hover:w-full transition-all duration-300"></span>
            </a>
            <a href="#testimonials" className="paragraph-font text-[var(--main-text-color)] hover:text-blue-500 transition-colors duration-300 relative group">
              Testimonials
              <span className="absolute bottom-[-5px] left-0 w-0 h-[2px] bg-blue-500 group-hover:w-full transition-all duration-300"></span>
            </a>
          </div>
        ...
        <div className={`md:hidden flex flex-col gap-6 overflow-hidden transition-all duration-300 ${isMenuOpen ? 'max-h-96 pt-8' : 'max-h-0'}`}>
          <a href="#home" className="paragraph-font text-[var(--main-text-color)] hover:text-blue-500 hover:pl-4 transition-all duration-300 py-2" onClick={() => setIsMenuOpen(false)}>
            Home
          </a>
          <a href="#projects" className="paragraph-font text-[var(--main-text-color)] hover:text-blue-500 hover:pl-4 transition-all duration-300 py-2" onClick={() => setIsMenuOpen(false)}>
            Projects
          </a>
          <a href="#testimonials" className="paragraph-font text-[var(--main-text-color)] hover:text-blue-500 hover:pl-4 transition-all duration-300 py-2" onClick={() => setIsMenuOpen(false)}>
            Testimonials
          </a>
